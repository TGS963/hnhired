...
- **Monitoring**: Slack/email webhook on ingest failure (GitHub Actions `if: failure()` step) and on extraction-failure rate > 10% per run. Google AI usage budget alerts configured at 50% / 100% of expected monthly spend.
